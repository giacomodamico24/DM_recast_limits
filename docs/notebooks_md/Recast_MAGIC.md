# Recasting MAGIC Upper Limits Notebook

This notebook reproduces the **recasting** of published MAGIC upper limits (ULs) on dark-matter (DM) annihilation channels.

In particular we will use as an example, the ULs of the following MAGIC publication

Acciari, Victor A., et al. "Combined searches for dark matter in dwarf spheroidal galaxies observed with the MAGIC telescopes, including new data from Coma Berenices and Draco." Physics of the Dark Universe 35 (2022): 100912.


https://arxiv.org/pdf/2111.15009 Figure 3
...
```python
# --- Load published MAGIC ULs for the reference channel (bb) and
#     interpolate them to a common DM mass grid --------------------

# Read ECSV table with columns: [mass, UL] for Coma Berenices (bb channel)
t = Table.read('Limits/magic_2022_comaberenices_ann_bb_median.ecsv')
...
# Here: from 100 GeV up to the max mass of the bb table, using 15 points
DM_masses = np.geomspace(100 * u.GeV, mDM[-1], 15)
...
# in linear space. If needed, consider log-log interpolation for smoother power-law-like curves.


# --- Load second benchmark channel (tau tau) and interpolate to the same grid --------------

t = Table.read('Limits/magic_2022_comaberenices_ann_tautau_median.ecsv')
...
t = Table.read('Limits/magic_2022_comaberenices_ann_WW_median.ecsv')
...
# Choose the *target* channel to recast to (MAGIC nomenclature: 'W' ≡ W+W-)
channel_to_recast = 'W'
...
```python
# --- Plotting & Visualization: overlay recasted ULs on published MAGIC figure -------------
# Goal:
#   1) Show our recasted bb → W limits (blue line) on top of the original MAGIC plot image.
#   2) Display, in a second (bottom) panel, the relative difference (%) between the
#      recasted limits and the published MAGIC W+W− ULs interpolated on our mass grid.

# Load the base image (screenshot of the published MAGIC figure for Coma Berenices, W channel)
image_path = './Paper_figures/MAGIC_W_Coma.png'
...
ax[0].imshow(
    img,
    extent=[np.log10(92), np.log10(98e3), np.log10(1e-28), np.log10(1e-20)],
    aspect='auto'
)

# Apply the same (log10) axis limits as the original plot to ensure perfect overlay
ax[0].set_xlim([np.log10(90), np.log10(1e5)])
ax[0].set_ylim([np.log10(1e-28), np.log10(1e-20)])
...
ax[0].plot(
    ...
    label=r'Recast limits'
)

# Legend styling: place it on transparent background above the image
ax[0].legend(loc=[0.3, 0.34], facecolor='none', edgecolor='none', handlelength=5.5)
...
custom_x_ticks = np.log10(np.array([1e2, 1e3, 1e4, 1e5]))
custom_y_ticks = np.log10([1e-28, 1e-27, 1e-26, 1e-25, 1e-24, 1e-23, 1e-22, 1e-21, 1e-20])
...
ax[0].set_yticklabels(
    [r'$10^{-28}$', r'$10^{-27}$', r'$10^{-26}$', r'$10^{-25}$',
     r'$10^{-24}$', r'$10^{-23}$', r'$10^{-22}$', r'$10^{-21}$', r'$10^{-20}$']
)
...
# (sigma_out is the MAGIC W+W− UL interpolated on our mass grid).
...
![png](Recast_MAGIC_files/Recast_MAGIC_13_0.png)
...
- channel to recast =   $W^+ W^-$
- benchmark channek =   $b \bar{b}$
- second benchmark channek =   $\mu^+ \mu^-$
...
# --- Load second benchmark channel (mu mu) and interpolate to the same grid --------------

t = Table.read('Limits/magic_2022_comaberenices_ann_mumu_median.ecsv')
...
# - DM_masses (Quantity) is the unified mass grid
# - sigma_in  contains ULs for bb on that grid
# - sigma_in2 contains ULs for mu mu on that grid (used to fit A_over_b)
# - sigma_out contains ULs for WW on that grid (used as target comparison)
#
# These three arrays can be passed to Recast_ULs along with the spectral models to
# fit the per-bin coefficients and compute the recasted ULs.
...
#   - sp_in:   reference channel (bb), provides the "source" ULs to be recast
#   - sp_out:  target channel (W+W-), where we want the recasted ULs
#   - sp_in_2: second benchmark channel (mu mu), used ONLY to fit A_over_b coefficients
sp_out  = [PrimaryFlux(mDM=im, channel=channel_to_recast) for im in DM_masses]
sp_in   = [PrimaryFlux(mDM=im, channel='b')              for im in DM_masses]
sp_in_2 = [PrimaryFlux(mDM=im, channel='mu')            for im in DM_masses]
...
#   - spectral_model_out:  models for target channel   (W)
#   - UL_values:           published ULs for reference channel (bb) on the common grid
#   - UL_values_2:         published ULs for second benchmark (mu mu) on the same grid
#   - spectral_model_in2:  models for second benchmark (mu mu)
#
# What happens inside:
#   1) ΔN_gamma are integrated per energy bin for each mass and channel.
#   2) A_over_b (∝ A_eff / sqrt(b)) per-bin coefficients are *fitted* so that
#      the predicted bb→mu ratio reproduces the published UL ratio sigma_in2 / sigma_in.
#   3) Those coefficients are then used to compute the bb→W recasting factor and
#      generate recasted ULs for the W channel.
r = Recast_ULs(
    DM_masses=DM_masses,
    ...
    spectral_model_out=sp_out,      # target (W)
    UL_values=sigma_in,             # ULs for bb
    UL_values_2=sigma_in2,          # ULs for mu mu (second benchmark)
    spectral_model_in2=sp_in_2      # spectra for mu mu
)
...
![png](Recast_MAGIC_files/Recast_MAGIC_20_0.png)
